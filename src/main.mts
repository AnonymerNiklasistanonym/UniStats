...
// Relative imports
import { createModuleEntry, createTable } from "./createHtmlParts.mjs";
import {
  fileExists,
  flattenArray,
  progressCalculator,
...
import {
  getClassesOfSemesterOfModule,
  getModuleAchievedCreditSum,
  getModuleCredits,
  getModuleCreditSum,
  getModuleExamTries,
  getModuleGradeAverage,
  getModules,
} from "./getModuleInfos.mjs";
...
import type {
  ...
  ModuleGroup,
  UniTemplate,
} from "./uniTemplate.mjs";
import type { HtmlTableCell } from "./createHtmlParts.mjs";
...
const inputCssFilePath: string = path.join(
  __dirname,
  "..",
  "styles",
  "default_table_style.css"
);
...
try {
  ...
  const dataFilePath = (await fileExists(inputJsonCustomDataFilePath))
    ? inputJsonCustomDataFilePath
    : inputDemoDataFilePath;
  const data = JSON.parse(
    await fs.readFile(dataFilePath, "utf8")
  ) as UniTemplate;
  if (
    !(
      data.module_groups.some((a) => a.modules !== undefined) ||
  ...
  const currentIsoDateString: string = new Date()
    .toISOString()
    .slice(0, ISO_DATE_LENGTH);

  const moduleData = getModules(data.module_groups);
  const moduleDataCredits = getModuleCredits(moduleData);
  const moduleDataExamTries = getModuleExamTries(moduleData);
  const credits: number = moduleDataCredits
    .filter((a) => a.grade !== undefined)
    .reduce((prev, curr) => prev + curr.credits, 0);
  const grade: number = moduleDataCredits
    .filter((a) => a.grade !== undefined)
    .reduce(
      (prev, curr) =>
        prev + ((curr.grade ? curr.grade : 0) / credits) * curr.credits,
      0
    );
  ...
  const examTryCount: ExamTryCount = {
    one: moduleDataExamTries.filter(
      (module) => module.tries === EXAM_TRY_COUNT_ONE
    ).length,
    three: moduleDataExamTries.filter(
      (module) => module.tries === EXAM_TRY_COUNT_THREE
    ).length,
    two: moduleDataExamTries.filter(
      (module) => module.tries === EXAM_TRY_COUNT_TWO
    ).length,
  };
   ...
   */
  const getProgressOfModuleList = (
    modules: ReadonlyArray<Module>,
    neededCredits?: number
  ): string => {
    const whole =
      neededCredits !== undefined ? neededCredits : getModuleCreditSum(modules);
    const gradeAverage = getModuleGradeAverage(modules);
    const creditSum = getModuleAchievedCreditSum(modules);
    const progress = progressCalculator(creditSum, whole, 0);

    return `Progress: ${progress.achieved}/${progress.whole} (${
      progress.percentage
    }%), Average grade: ${parseFloat(gradeAverage.toFixed(1))}`;
  };
   ...
   */
  const createModuleRowForTable = (module: Module): HtmlTableCell[] => [
    { content: createModuleEntry(module) },
   ...
   * @param progress
   */
  const createCatalogSectionsForTable = (
    catalog: Catalog,
    progress = true
  ): HtmlTableCell[][] => {
    const modules: Module[] =
      catalog.modules !== undefined ? catalog.modules : [];
    const progressCells: HtmlTableCell[][] = progress
      ? [
          [
            {
   ...
   * @param progress
   */
  const createModuleGroupSectionForTable = (
    moduleGroup: Readonly<ModuleGroup>,
    progress = true
  ): HtmlTableCell[][] => {
    const modules = getModules([moduleGroup]);
    const neededCredits: number =
      (moduleGroup.catalogs !== undefined
        ? moduleGroup.catalogs.reduce(
            (sum, catalog) => sum + catalog.credits,
            0
          )
        : 0) +
      modules
        .filter((module) => module.info.catalog === undefined)
   ...
   */
  const createSemesterProgressRows = (
    tableCellOffset: number,
    semesterCellCount: number /* , modules: UniTemplate.Module[] */
  ): HtmlTableCell[][] => {
    const semesters = range(semesterCellCount, 1);
    const moduleCreditGradeInfo = getModuleCredits(moduleData);
    const semesterStats = semesters.map((semester) => {
      ...
      const creditSumSemester = semesterModules.reduce(
        (sum, currentModule) => sum + currentModule.credits,
        0
      );
      ...
      const credGradeProg: ProgressCreditsGradeSemester =
        semesterModules.reduce(
          (result: ProgressCreditsGradeSemester, currentModuleInfo) => ({
            credits: result.credits + currentModuleInfo.credits,
            grade:
              result.grade +
              ((currentModuleInfo.grade ? currentModuleInfo.grade : 0) /
                creditSumSemester) *
      ...
      return {
        averageGradePerSemester: `${parseFloat(
          credGradeProg.grade.toFixed(1)
        )}`,
        pointsPerSemester: {
        ...
        hoursPerSemesterWeek: {
          registered:
            (creditSumAllSemester * hoursPerEtcsCredit) / weeksPerSemester,
          achieved:
            (credGradeProg.credits * hoursPerEtcsCredit) / weeksPerSemester,
        },
      };
    });
    ...
    return [
      ...
      [
        {
          content: "Points per semester (achieved/registered)",
          colSpan: tableCellOffset,
        },
        ...semesterStats.map((a) => ({
          content: `${a.pointsPerSemester.achieved}/${
            a.pointsPerSemester.registered
          } (${parseFloat(
            (
              (a.pointsPerSemester.achieved / a.pointsPerSemester.registered) *
              100
            ).toFixed(1)
          )}%)`,
        })),
      ...
      [
        {
          content:
            "Registered hours per week in lecture period (achieved/registered)",
          colSpan: tableCellOffset,
        },
        ...semesterStats.map((a) => ({
          content: `${a.hoursPerSemesterWeek.achieved}/${a.hoursPerSemesterWeek.registered}`,
        })),
      ],
    ];
  };
  ...
  const htmlTable = createTable(
    [
      [
        { content: "", headerCell: true, colSpan: tableCellCountWoSem },
        {
          content: `Semester (${renderSemester(
            semesterInfo.startSemester,
            semesterInfo.startYear
          )} - ${renderSemester(
            semesterInfo.latestSemester,
            semesterInfo.latestYear
          )} )`,
          headerCell: true,
    ...
    [
      ...flattenArray(
        data.module_groups.map((moduleGroup) =>
          createModuleGroupSectionForTable(moduleGroup, true)
        )
      ),
      ...createSemesterProgressRows(
        tableCellCountWoSem,
        semesterCount /* , moduleData */
      ),
    ]
  );

  const creditProgress = progressCalculator(credits, data.needed_credits, 0);
  const htmlTableStats = createTable(
    [[]],
    [
      [
        { content: "Currently accumulated credits" },
        {
          content: `${creditProgress.achieved}/${creditProgress.whole} (${creditProgress.percentage}%)`,
        },
      ],
      [
      ...
      [{ content: "Exam 2nd tries" }, { content: `${examTryCount.two}` }],
      [{ content: "Exam 3rd tries" }, { content: `${examTryCount.three}` }],
    ]
  );
  ...
  const htmlDocumentContent = `<!DOCTYPE html><html><head><style>${cssContent}</style><meta charset="UTF-8"></head><body><h2>${data.title}</h2><h3>${data.name.surname}, ${data.name.first_name} - ${data.matriculation_number} (${data.field_of_study}, ${data.current_semester}. semester)</h3><h4>${currentIsoDateString}</h4>${htmlTable}<br>${htmlTableStats}</body></html>`;

  // Write table to local file
  await fs.writeFile(outputHtmlFilePath, htmlDocumentContent);
  // eslint-disable-next-line no-console
  console.info(
    `HTML table was written to '${outputHtmlFilePath}' in ${performance.now()} ms using ${dataFilePath}`
  );
} catch (err) {
  console.error(err);
  process.exit(1);
